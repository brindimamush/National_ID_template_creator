...
# ---- OCR PROCESSING (images swapped) ----
def process_image3_image4_with_ocr(template, image3_path, image4_path):
    try:
        log("Step 3", "Processing image 3 and 4 with OCR...")

        regions = [
            {"type": "ocr", "source_img": "img4", "snapshot": (231, 2451, 957, 111), "paste": (355, 385)},  # swapped
            {"type": "paste", "source_img": "img4", "snapshot": (525, 2628, 825, 285), "paste": (417, 439, 220, 75)},  # swapped
            {"type": "paste", "source_img": "img3", "snapshot": (1248, 2052, 546, 96), "paste": (1056, 462, 161, 30)},  # swapped
        ]
...
def write_pdf_blocks_on_template(pdf_doc, template_img):
    ...
    block_to_png_mapping = [
        ...
        {"pdf_block_index": 35, "png_point": {"x": 961, "y": 362, "w": 377, "h": 82}},
    ...
    for mapping in block_to_png_mapping:
        ...
        # Handle block 34 font sizing logic
        if idx == 34:
            ...
            if (full_text_bbox[2] - full_text_bbox[0]) > max_width:
                try:
...
# ---- DATE FUNCTIONS (use image_4.png) ----
def extract_dates_from_image(image_path):
    try:
        ...
        log("Step 4.5", f"Date OCR error: {e}")
        return "", ""
...
def process_dates_from_image(template_img, image3_path):
    try:
        log("Step 4.5", "Extracting and placing dates from image 4...")
        eth_date, eng_date = extract_dates_from_image(image3_path.replace("image_3", "image_4"))
        try:
...
def main_process(pdf_path, template_path, output_path, a4_template_path, output_a4_path):
    ...
    # Process and add the dates (swapped)
